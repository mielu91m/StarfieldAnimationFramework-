...
namespace Tasks
{
	...
	static void PerformInputProcessingHookImpl(const RE::PlayerCamera* a_camera, const RE::InputEvent* a_queueHead)
	{
		...
		if (auto* mgr = Animation::GraphManager::GetSingleton()) {
			...
			// Ten sam sygnał co PlayerUpdate/SceneRect: przy aktywnym pumpie grafu nie dublekuj SAF z ścieżki inputu —
			// w przeciwnym razie (~co klatkę) drugie SetDeltaTime/Generate = wyraźne przeskoki czasu klipu.
			if (!Animation::GraphManager::IsPrimaryGraphPumpActive()) {
