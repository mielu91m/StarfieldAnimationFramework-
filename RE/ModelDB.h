...
namespace RE
{
	namespace ModelDB
	{
		...
		/// Jak GetEntry, ale oba adresy z RVA (moduleBase + RVA). Gdy masz tylko RVA z IDA.
		inline Entry* GetEntryWithRVA(const char* filename, std::uint32_t a_getEntryRVA, std::uint32_t a_decRefRVA, std::uintptr_t a_moduleBase)
		{
			constexpr std::uint64_t flag = 0x3;
			UnkOut out;
			out.unk00 = static_cast<std::uint32_t>(flag & 0xFFFFFFu);
			...
			Entry* entry = nullptr;
			auto* fnGetEntry = reinterpret_cast<int(*)(const char*, Entry**, UnkOut&)>(a_moduleBase + a_getEntryRVA);
			fnGetEntry(filename, &entry, out);
