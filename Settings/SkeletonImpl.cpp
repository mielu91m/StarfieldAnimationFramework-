...
namespace Settings
{
	// Opakowanie ModelDB::GetEntry w SEH. ID z INI; gdy ModelDBGetEntryRVA ustawione – używamy RVA zamiast ID.
	static RE::ModelDB::Entry* SafeGetModelDBEntry(const char* filename)
	{
		...
		const std::uint32_t getEntryRva = Animation::GraphManager::GetModelDBGetEntryRVA();
...
#ifdef _WIN32
		__try {
			// Gdy oba RVA są ustawione, użyj RVA zamiast ID (najszybsza ścieżka)
			if (getEntryRva != 0 && decRefRva != 0) {
				return RE::ModelDB::GetEntryWithRVA(filename, getEntryRva, decRefRva, base);
			}
			// Gdy tylko DecRef RVA jest ustawione, użyj ID dla GetEntry i RVA dla DecRef
			if (decRefRva != 0) {
				return RE::ModelDB::GetEntryWithDecRefRVA(filename, id1, decRefRva, base);
			}
			// Fallback: użyj obu ID
