...
{
    // Globalny wskaźnik ostatnio zboundowanej VM
	...
	// =========================================================================
	// RebindAfterLoad – wywołaj po wczytaniu zapisu gry.
	// Po LoadGame Papyrus VM jest nową instancją – native bindingi są tracone.
	// Ta funkcja pobiera aktualny VM singleton i rebinduje wszystkie funkcje.
	// =========================================================================
	void RebindAfterLoad()
	{
		auto* vm = RE::BSScript::Internal::VirtualMachine::GetSingleton();
		if (!vm) {
			SAF_LOG_WARN("[RebindAfterLoad] VM is null, cannot rebind");
			return;
		}
		SAF_LOG_INFO("[RebindAfterLoad] Rebinding SAFScript native functions after load");
		Bind(vm);
	}
		...
		bool Bind(RE::BSScript::IVirtualMachine* a_vm)
    {
        ...
        try {
            ...
            g_lastBoundVM.store(a_vm);  // Zapamiętaj ostatnio zboundowaną VM
            return true;
        } catch (const std::exception& e) {
    ...
    bool NeedsRebind()
    {
        ...
        auto* lastBoundVM = g_lastBoundVM.load();
        // Zrzutuj oba wskaźniki do IVirtualMachine dla porównania
        return currentVM && static_cast<RE::BSScript::IVirtualMachine*>(currentVM) != lastBoundVM;
    }
}